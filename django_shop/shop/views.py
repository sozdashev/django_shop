...
def order_product(request, product_id):
    ...
    if form.is_valid():
        try:
            cart_product = Order.objects.get(user_fk = request.user.id, accepted = False, product_fk = product.id)
        except:
            cart_product = ''
        if cart_product:
            ...
            order.cost = product.price * order.count
            order.save()
...
class Product_Create(PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        form = ProductForm
        return render(request, 'shop/product_add.html', context={'form':form})
...
class Profile_Update(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'profile.html', context={'form':form, 'obj':data_obj})
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(request,'profile.html', context={'form':bound_form, 'obj':data_obj}),
